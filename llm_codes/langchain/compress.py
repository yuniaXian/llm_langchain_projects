...
def cal_sentence_info(model, tokenizer, config,sentence):
    ...
    ## input_ids = [60670, 60781,  32021, 38232, 76, 3423, 214, 2]
    ## target_ids =               [ 32021, 38232, 76, 3423, 214, 2]

    # I want to compute p(sentence | model, beginning tokens) logp -> entropy
    ...
    for i in range(0,target_length):
        ...
        # info entropy information entropy of the first token, loss
        results.append(-1*math.log(p))
    score=np.mean(results)
    return score
def cut_sentences(content):
	# chars indicating endding
	end_flag = ['?', '!', '.', '？', '！', '。', '…']
